...
public class RioVVoidHolder extends Item
{
	/** field for checking if the bucket has been filled. */
	private int isFull;

	public RioVVoidHolder(int par1, int par2)
	{
		super(par1 - 256);
		this.maxStackSize = 1;
		this.isFull = par2;
			...
			if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
			{
				...
				if (this.isFull == 0)
				{
					...
					if (par2World.getBlockMaterial(i, j, k) == Material.water && par2World.getBlockMetadata(i, j, k) == 0)
					{
						...
						if (--par1ItemStack.stackSize <= 0)
						{
							return new ItemStack(Items.voidHolderFilled);
						}

						if (!par3EntityPlayer.inventory.addItemStackToInventory(new ItemStack(Items.voidHolderFilled)))
						{
							par3EntityPlayer.dropPlayerItem(new ItemStack(Items.voidHolderFilled.itemID, 1, 0));
						}
				...
				else
				{
					if (this.isFull < 0)
					{
						return new ItemStack(Items.voidHolder);
					}
	...
	public boolean tryPlaceContainedLiquid(World par1World, int par2, int par3, int par4)
	{
		...
		else
		{
			...
			{
				if (par1World.provider.isHellWorld && this.isFull == Blocks.voidMoving.blockID)
				{
	...
	{
		if(itemID == Items.voidHolder.itemID) this.itemIcon = i.registerIcon(Util.MOD_ID + ":" + "voidHolder");
		if(itemID == Items.voidHolderFilled.itemID) this.itemIcon = i.registerIcon(Util.MOD_ID + ":" + "voidHolderFilled");
	}
}
