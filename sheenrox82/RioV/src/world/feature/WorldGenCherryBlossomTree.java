...
public class WorldGenCherryBlossomTree extends WorldGenerator implements IWorldGenerator
{
	...
	{
		this(par1, 4, 1, 0, false);
	}
	public WorldGenCherryBlossomTree(boolean par1, int par2, int par3, int par4, boolean par5)
	{
		super(par1);
	...
	public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5)
	{
		...
		if (par4 >= 1 && par4 + l + 1 <= 256)
		{
			...
			else
			{
				...
				boolean isSoil = (soil != null && soil.canSustainPlant(par1World, par3, par4 - 1, par5, ForgeDirection.UP, (BlockRioVSapling)Blocks.cherryBlossomSapling));
				if (isSoil && par4 < 256 - l - 1)
				{
						...
						for (j2 = par3 - i2; j2 <= par3 + i2; ++j2)
						{
							...
							for (int l2 = par5 - i2; l2 <= par5 + i2; ++l2)
							{
								...
								if (Math.abs(k2) != i2 || Math.abs(i3) != i2 || par2Random.nextInt(2) != 0 && k1 != 0)
								{
									...
									{
										/** Custom leaf block **/
										this.setBlockAndMetadata(par1World, j2, j1, l2, Blocks.cherryLeaf.blockID, this.metaLeaves);
									}
								}
							}
						}
					...
					for (j1 = 0; j1 < l; ++j1)
					{
						...
						{
							/** Custom Log block **/
							this.setBlockAndMetadata(par1World, par3, par4 + j1, par5, Blocks.cherryLog.blockID, this.metaWood);
						}
					}
				}
				return true;
			...
			IChunkProvider chunkGenerator, IChunkProvider chunkProvider)
	{
		for(int i = 0; i < 10; i++)
		{
			...
			(new WorldGenCherryBlossomTree(false, 4, 1, 0, false)).generate(world, random, xCoord, yCoord, zCoord);
		}
	}
}