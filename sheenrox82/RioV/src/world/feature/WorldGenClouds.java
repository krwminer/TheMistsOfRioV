...
public class WorldGenClouds extends WorldGenerator
{
	...
	double scaleWidth = 5D;
	double leafDensity = 6D;
	...
	public WorldGenClouds(boolean par1)
	{
		super(par1);
	}
		...
		for (aint1[par5] = aint[par5]; j1 <= i1; ++j1)
		{
			...
			while (k1 <= i1)
			{
				...
				else
				{
					...
					if (l1 != 0 && l1 != Blocks.skycloud.blockID)
					{
						++k1;
					}
					else
	 ...
	 float layerSize(int par1)
	{
	 ...
	 void generateLeafNode(int par1, int par2, int par3)
	 {
		 ...
		 for (int i1 = par2 + this.leafDistanceLimit; l < i1; ++l)
		 {
			 ...
			 this.genTreeLayer(par1, l, par3, f, (byte)1, Blocks.skycloud.blockID);
		 }
	 }
	 ...
	 int checkBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger)
	 {
		 ...
		 else
		 {
			 ...
			 for (j = aint2[b1] + b4; i != j; i += b4)
			 {
				 ...
				 if (k != 0 && k != Blocks.skycloud.blockID)
				 {
					 break;
				 }
			 }
