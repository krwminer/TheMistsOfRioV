...
public class BlockSanctuatitePortal extends BlockBreakable
{
	public BlockSanctuatitePortal(int par1)
	{
		super(par1, Util.MOD_ID + ":" + "sanctuatitePortal", Material.portal, false);
	...
	public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
	{
		...
		else
		{
			...
			for (l = -1; l <= 2; ++l)
			{
				for (i1 = -1; i1 <= 3; ++i1)
				{
					...
					if (l != -1 && l != 2 || i1 != -1 && i1 != 3)
					{
						...
						if (flag)
						{
							if (j1 != Block.blockNetherQuartz.blockID)
							{
								return false;
							}
						}
						else if (j1 != 0 && j1 != Blocks.sanctuatiteFire.blockID)
						{
							return false;
						}
					}
			...
			for (l = 0; l < 2; ++l)
			{
				for (i1 = 0; i1 < 3; ++i1)
				{
					par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, Blocks.sanctuatitePortal.blockID, 0, 2);
				}
			}
			return true;
		}
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5)
	{
		...
		else
		{
			...
			if (j1 == 3 && par1World.getBlockId(par2, i1 + j1, par4) == Block.blockNetherQuartz.blockID)
			{
				...
				{
					if ((par1World.getBlockId(par2 + b0, par3, par4 + b1) != Block.blockNetherQuartz.blockID || par1World.getBlockId(par2 - b0, par3, par4 - b1) != this.blockID) && (par1World.getBlockId(par2 - b0, par3, par4 - b1) != Block.blockNetherQuartz.blockID || par1World.getBlockId(par2 + b0, par3, par4 + b1) != this.blockID))
					{
						par1World.setBlockToAir(par2, par3, par4);
					}
				}
	...
	public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity)
	{
		if ((par5Entity.ridingEntity == null) && (par5Entity.riddenByEntity == null) && ((par5Entity instanceof EntityPlayerMP)))
		{
			...
			else if (thePlayer.dimension != Config.sanctuatiteID)
			{
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, Config.sanctuatiteID, new TeleporterSanctuatite(thePlayer.mcServer.worldServerForDimension(Config.sanctuatiteID)));
			}
			else 
			{
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new TeleporterSanctuatite(thePlayer.mcServer.worldServerForDimension(0)));
			}
		}
	}
	@SideOnly(Side.CLIENT)
